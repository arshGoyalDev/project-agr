...
use iced::widget::{
    button, Button, Canvas, Column, Container, MouseArea, Row, Space, Text, TextInput,
};
    ...
    pub fn view(&self) -> Element<'_, Message> {
        ...
        for (i, tab) in self.tabs.iter().enumerate() {
            ...
            let label_btn = Button::new(
                Text::new(if is_active { format!("{}", display_title) } else { display_title })
                .size(14.0),
            )
        ...
        if can_go_back { back_btn = back_btn.on_press(Message::GoBack); }

        let mut forward_btn = Button::new(Text::new(">")).style(button::text);
        if can_go_forward { forward_btn = forward_btn.on_press(Message::GoForward); }
