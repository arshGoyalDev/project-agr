use crate::utils::{Text, Element, Node};
...
impl HTMLParser {
  ...
  fn implicit_tags(&mut self, tag: Option<&str>) {
    loop {
      let open_tags: Vec<String> = self.unfinished
        .iter()
      ...
      } else if open_tags == vec!["html", "head"] && !matches!(tag, Some("/head")) && !tag.map(|t| HEAD_TAGS.contains(&t)).unwrap_or(false) {
        self.add_tag("/head".to_string());
      } else {
        break;
      }
